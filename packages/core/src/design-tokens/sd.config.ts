...
const THEMES: readonly ThemeConfig[] = [
    ...
    },
    {
        ...
        manifest: {
            destination: "../../theme/generated/bp7ThemeTokensLight.ts",
            exportName: "BLUEPRINT_BP7_THEME_TOKENS_LIGHT",
        },
    },
    {
        name: "next-dark",
        ...
        // Portals may receive the color-scheme class on either an ancestor or their own root. An explicit light scheme
        // belongs to a nested provider and must win over a legacy dark ancestor.
        selector:
            '[data-bp-color-scheme=\"dark\"] .bp-next:where(:not([data-bp-color-scheme=\"light\"])),\n' +
            '.bp-next[data-bp-color-scheme=\"dark\"],\n' +
            '.bp6-dark .bp-next:where(:not([data-bp-color-scheme=\"light\"])),\n' +
            '.bp-next.bp6-dark:where(:not([data-bp-color-scheme=\"light\"]))',
        destination: "next-tokens-dark.css",
        manifest: {
            destination: "../../theme/generated/bp7ThemeTokensDark.ts",
            exportName: "BLUEPRINT_BP7_THEME_TOKENS_DARK",
        },
    },
];
...
/** Generates a typed public-token manifest from the same transformed values used by BP7 CSS. */
const formatPublicTokenManifest = (tokens: readonly TransformedToken[], exportName: string): string => {
    if (!/^[A-Z][A-Z0-9_]*$/.test(exportName)) {
        throw new Error(`Invalid token manifest export name: ${exportName}`);
    }

    const tokenMap = buildTokenMap(tokens);
    const fallbackMap = makeFallbackMap(tokens, tokenMap);
    const publicTokens = tokens
        .map(token => ({ classification: classifyToken(token, fallbackMap), token }))
        .filter(({ classification }) => !isPrivateTokenName(classification.name));
    const manifest = Object.fromEntries(
        publicTokens.map(({ classification, token }) => [
            `--${classification.name}`,
            applyRoleToValue(classification.modernValue ?? classification.fallbackValue, token),
        ]),
    );
    ...
    return (
        ...
        " */\n\n" +
        `export const ${exportName} = ${JSON.stringify(manifest, null, 4)} as const;\n`
    );
};

const isPrivateTokenName = (tokenName: string): boolean =>
    tokenName === "bp-private" || tokenName.startsWith("bp-private-");
...
const initializeStyleDictionary = (sd: typeof StyleDictionary): void => {
    ...
    sd.registerFormat({
        name: "bp/typescript/token-manifest",
        format: ({ dictionary, options }) => {
            const exportName = parseObject(options)?.exportName;
            if (typeof exportName !== "string") {
                throw new Error("Token manifest format requires an exportName option.");
            }
            return formatPublicTokenManifest(dictionary.allTokens, exportName);
        },
    });
};
    ...
    platforms: {
        css: {
            ...
            files: [
                ...
                ...(theme.manifest === undefined
                    ? []
                    : [
                          {
                              destination: theme.manifest.destination,
                              format: "bp/typescript/token-manifest",
                              options: {
                                  exportName: theme.manifest.exportName,
                              },
                          },
                      ]),
            ],

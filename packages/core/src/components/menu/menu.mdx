...
Set `popoverProps={{ safePolygon: true }}` on a parent **MenuItem** to keep its submenu open while the
pointer moves diagonally toward it. To also prevent accidentally hovering neighboring items, use
`popoverProps={{ safePolygon: { blockPointerEvents: true } }}`. See
[PopoverNext safe polygons](#core/components/popover-next.safe-polygons) for the available options.

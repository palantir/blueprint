...
Many Blueprint components accept an `icon` prop which can be specified as either a string icon name (e.g. `"refresh"`) or a JSX element such as a dynamically-loaded `<Icon>` component. Both approaches render the same result — the string shorthand is more concise, while the JSX form allows customizing icon props like `size`, `intent`, or `color`.

@reactCodeExample IconUsageExample

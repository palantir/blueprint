...
Set `intent` to color a tab: the intent applies to the tab title, its `icon`, its `tagContent` tag, and the
indicator shown while the tab is selected. A tab without an `intent` keeps the default colors and turns
primary while selected. Disabled tabs ignore `intent` and keep the disabled color.

...
describe("<Tabs>", () => {
    ...
    describe("when state is managed internally", () => {
        it("sets initialSelectedTabIndex", () => {
            ...
            const wrapper = mount(<Tabs initialSelectedTabIndex={TAB_INDEX_TO_SELECT}>{getTabsContents()}</Tabs>);
            assert.isTrue(
                wrapper
                    .find(Tab)
                    .at(TAB_INDEX_TO_SELECT)
                    .prop("isSelected"),
            );
        });
        ...
        it("does not reset selected tab to initialSelectedTabIndex after a selection is made", () => {
            const TAB_INDEX_TO_SELECT = 2;
            const wrapper = mount(<Tabs initialSelectedTabIndex={1}>{getTabsContents()}</Tabs>);
    ...
    describe("when state is managed externally (selectedTabIndex prop is provided)", () => {
        ...
        it("defaults to first tab if invalid index provided", () => {
            const tabs = mount(<Tabs selectedTabIndex={7}>{getTabsContents()}</Tabs>);
        ...
        it("doesn't switch tabs automatically", () => {
            const TAB_INDEX_TO_SELECT = 1;
            const tabs = mount(<Tabs selectedTabIndex={0}>{getTabsContents()}</Tabs>);
        ...
        it("indicator moves correctly if tabs switch externally via the selectedTabIndex prop", done => {
            const TAB_INDEX_TO_SELECT = 1;
            const wrapper = mount(<Tabs selectedTabIndex={0}>{getTabsContents()}</Tabs>, {
                attachTo: testsContainerElement,
            });
    ...
    function getTabsContents(): Array<React.ReactElement<any>> {
        ...
        return [
            ...
            <TabPanel key={1}>first panel</TabPanel>,
            <TabPanel key={2}>second panel</TabPanel>,
            <TabPanel key={3}>third panel</TabPanel>,
        ];
    }
});

 ...
 *
 * This script replaces compile-docs-data.mjs. Instead of running Documentalist to produce
 * a monolithic docs.json, it reads nav.json and the MDX files on disk to generate:
...
const generatedDir = resolve(cwd(), "./src/generated");
...
try {
    ...
    generatePageRegistry(navConfig, mdxFileMap, routeMap, allRefs);
    generateNpmVersions();
 ...
 *
 * @returns {Map<string, string>} pageRef -> absolute file path
 */
function discoverMdxFiles() {
    ...
    for (const pkg of ALL_DOC_PACKAGES) {
        const pkgSrcDir = resolve(monorepoRoot, "packages", pkg, "src");
...
// ---------------------------------------------------------------------------
// Route map (reused from compile-docs-data.mjs)
// ---------------------------------------------------------------------------
 ...
 */
function generatePageRegistry(navConfig, mdxFileMap, routeMap, allRefs) {
    const entries = [];
    const warnings = [];
    ...
    for (const ref of allRefs) {
        ...
        const importPath = relative(generatedDir, filePath).replace(/\\/g, "/");
        // Source path relative to monorepo root (for "Edit this page" links)
        const sourcePath = relative(monorepoRoot, filePath).replace(/\\/g, "/");
    ...
    const lines = [
        "// Auto-generated by generate-page-registry.mjs — do not edit",
    ...
    writeFileSync(join(generatedDir, "pageRegistry.ts"), lines.join("\n"));
...
function generateNpmVersions() {
    const lines = [
        ...
        "}",
        "",
        "export const npmVersions: Record<string, NpmPackageInfo> = {",
    ];

    for (const pkg of LIBRARY_PACKAGES) {
        const pkgJsonPath = resolve(monorepoRoot, "packages", pkg, "package.json");
        if (!existsSync(pkgJsonPath)) continue;

        const pkgJson = JSON.parse(readFileSync(pkgJsonPath, "utf-8"));
        lines.push(`    ${JSON.stringify(pkgJson.name)}: {`);
        lines.push(`        name: ${JSON.stringify(pkgJson.name)},`);
        lines.push(`        version: ${JSON.stringify(pkgJson.version)},`);
        lines.push(`    },`);
    }
    ...
    writeFileSync(join(generatedDir, "npmVersions.ts"), lines.join("\n"));

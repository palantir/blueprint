...
import { AnchorButton, Button, Classes, Pre, Tooltip } from "@blueprintjs/core";
...
export const CodeExample: React.FC<CodeExampleProps> = props => {
    const { children, className, id, previewCode, sourceCode, sourceUrl, ...rest } = props;
    ...
    return (
        <div className={classes} data-example-id={id} {...rest}>
            ...
            <div className="docs-code-example-toolbar">
                ...
                <Tooltip content={isMinimized ? "Expand code" : "Minimize code"} position="top">
                    <Button icon={isMinimized ? "maximize" : "minimize"} onClick={toggleMinimized} />
                </Tooltip>
                <CopyToClipboardButton text={isMinimized ? previewCode : sourceCode} />
                {sourceUrl && <GitHubSourceButton sourceUrl={sourceUrl} />}
            </div>
            <div>
...
function CopyToClipboardButton({ text }: { text: string }) {
    ...
    return (
        <Tooltip content="Copy to clipboard" position="top">
            <Button icon="duplicate" onClick={onClick} />
        </Tooltip>
    );
}
...
function CodeSandboxButton({ id, sourceCode }: { id: string; sourceCode: string }) {
    ...
    return (
        <form action="https://codesandbox.io/api/v1/sandboxes/define" method="POST" target="_blank">
            ...
            <Tooltip content="Open in CodeSandbox" position="top">
                <Button icon={<CodeSandboxIcon />} type="submit" />
            </Tooltip>
        </form>
    );
}

function GitHubSourceButton({ sourceUrl }: { sourceUrl: string }) {
    return (
        <Tooltip content="View source on GitHub" position="top">
            <AnchorButton icon="code" href={sourceUrl} target="_blank" />
        </Tooltip>
    );
}

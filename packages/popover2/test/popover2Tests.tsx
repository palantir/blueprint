...
describe.only("<Popover2>", () => {
    ...
    describe.only("key interactions on Button target", () => {
        const SPACE_KEYSTROKE = { keyCode: Keys.SPACE, which: Keys.SPACE };

        describe("Enter key down opens click interaction popover", () => {
            it("when autoFocus={true}", () => {
                wrapper = renderPopover({ autoFocus: true });
                wrapper
                    .simulateTarget("focus")
                    .simulateTarget("keyDown", SPACE_KEYSTROKE)
                    .simulateTarget("keyUp", SPACE_KEYSTROKE);
                // wrapper.find("[data-testid='target-button']").simulate("keyDown", SPACE_KEYSTROKE);
                // wrapper.find("[data-testid='target-button']").simulate("keyUp", SPACE_KEYSTROKE);
                wrapper.assertIsOpen();
            });

            it.only("when autoFocus={false}", () => {

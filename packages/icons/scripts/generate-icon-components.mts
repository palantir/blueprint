 ...
 * generated by fantasticon (in other words, run ./generate-icon-fonts first).
 */
...
import { parse, type SvgParserNode } from "svg-parser";
...
Handlebars.registerHelper("pascalCase", (iconName: string) => pascalCase(iconName));
...
const iconPaths: Record<IconRasterSize, Record<string, string>> = {
    [ICON_SIZES[0]]: {},
    [ICON_SIZES[1]]: {},
};
 ...
 */
function parseIconGlyphs(iconFontSvgDocument: string, cb: (iconName: string, iconPath: string) => void): void {
    const rootNode: SvgParserNode = parse(iconFontSvgDocument);
    const fontOrSvg = rootNode.children?.[0];
    const defs = fontOrSvg?.children?.[0];
    const font = defs?.children?.[0];
    const glyphs = font?.children?.filter(node => node.tagName === "glyph") ?? [];

...
menu: Select
name: Select
route: /select/components/select
---
...
<Callout intent="primary" title="Disabling a Select">
...
these functions. See the code sample in [Item Renderer API](#select/select-component.item-renderer) below for usage.
...
The input value can be controlled with the `query` and `onQueryChange` props.
_Do not use `inputProps` for this;_ the component ignores `inputProps.value` and
`inputProps.onChange` in favor of `query` and `onQueryChange` (as noted in the
prop documentation).
...
<Interface name="IItemRendererProps" />
...
<Interface name="IItemListRendererProps" />

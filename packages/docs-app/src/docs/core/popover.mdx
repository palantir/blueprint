...
import { PopoverPositionExample, SideLabel, AlignmentLabel } from "./examples/popoverPositionExample";
...
- Which <SideLabel /> of the target the popover should render on.
- The popover's <AlignmentLabel /> relative to the target.
...
<pre className="docs-popover-position-value-code-block">
    <SideLabel />_<AlignmentLabel />
</pre>

The following example shows all supported `Position` values and how each behaves
in practice. Note that if <AlignmentLabel />
is ommitted, the popover will align to the __center__ of the target.

<PopoverPositionExample />

#### Automatic positioning

The Popover's `position` can also be chosen _automatically_ via `"auto"`,
`"auto-start"`, or `"auto-end"`. All of these options choose and continually
update the <SideLabel /> for you to avoid overflowing the boundary element (when
scrolling within it, for instance). The options differ in how they handle <AlignmentLabel />:

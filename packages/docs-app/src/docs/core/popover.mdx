...
import { PopoverInteractionKindExample } from "./examples/popoverInteractionKindExample";
import { PopoverMinimalExample } from "./examples/popoverMinimalExample";
import { PopoverPortalExample } from "./examples/popoverPortalExample";
import { PopoverPositionExample } from "./examples/popoverPositionExample";
import { PopoverSizingExample } from "./examples/popoverSizingExample";
...
```tsx
<span className="@ns-popover-wrapper">
    <span className="@ns-popover-target">
...
<Callout intent="warning" title="Button targets">
...
- Which <span className="docs-popover-position-label-side">__side__</span> of the target the popover should render on.
- The popover's <span className="docs-popover-position-label-alignment">__alignment__</span> relative to the target.

These two attributes can be expressed with a single value having the following structure:

<pre className="docs-popover-position-value-code-block">
    <span className="docs-popover-position-label-side">[SIDE]</span>_<span className="docs-popover-position-label-alignment">[ALIGNMENT]</span>
</pre>

The following example shows all supported `Position` values and how each behaves in practice. Note that if <strong><code>_<span className="docs-popover-position-label-alignment">[ALIGNMENT]</span></code></strong> is ommitted, the popover will align to the __center__ of the target.

<PopoverPositionExample />

#### Automatic positioning

The Popover's `position` can also be chosen _automatically_ via `"auto"`, `"auto-start"`, or `"auto-end"`. All of these options choose and continually update the <span className="docs-popover-position-label-side">__side__</span> for you to avoid overflowing the boundary element (when scrolling within it, for instance). The options differ in how they handle <span className="docs-popover-position-label-alignment">__alignment__</span>:
...
<Callout intent="warning" title="Disabling controlled popovers">
    ...
    public render() {
        let popoverContent = (
            <div>
                <h5>Popover Title</h5>
                <p>...</p>
                <button className="@ns-button @ns-popover-dismiss">Close popover</button>
            </div>
        );
...
<Callout intent="primary" title="Conditionally styling popover targets">
...
<Callout intent="danger" title="Dangerous edge case">
...
```tsx
<Popover content={<div className="custom-class">...</div>}>
    ...
</Popover>
```

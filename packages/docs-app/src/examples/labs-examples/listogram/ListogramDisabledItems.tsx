...
const ITEMS: ListogramItem[] = [
    ...
    { count: 3, disabled: true, title: "Apricots" },
    { count: 2, title: "Pears" },
    { count: 2, disabled: true, title: "Oranges" },
    { count: 1, title: "Cherries" },
].map((item, index) => ({ id: index.toString() as ListogramItemId, ...item }));

export default function ListogramDisabledItems() {
    return <Listogram items={ITEMS} title="Fruits" selectionKind="toggle" />;
}

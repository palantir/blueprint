...
export type { DateInputNextProps };
 ...
 *
 * Uses PopoverNext (Floating UI) instead of Popover (Popper.js).
 *
 * @see https://blueprintjs.com/docs/#datetime/date-input
 */
export const DateInputNext: React.FC<DateInputNextProps> = memo(function DateInputNext(props) {
    const {
    ...
    // These hooks only read format/parse-related props which are identical across both prop types.
    const formatDateString = useDateFormatter(props as unknown as DateInputProps, locale);
    const parseDateString = useDateParser(props as unknown as DateInputProps, locale);
    ...
    const renderTarget = useCallback(
        ({ isOpen: targetIsOpen, ref, ...targetProps }: PopoverTargetProps & PopoverClickTargetHandlers) => {
            return (
                <InputGroup
                    ...
                    tagName={popoverNextProps.targetTagName}
                    type="text"
                    role="combobox"
                    {...targetProps}
                    {...inputProps}
        ...
        [
            ...
            placeholder,
            popoverId,
            popoverNextProps.targetTagName,
            rightElement,
    ...
    return (
        <PopoverNext
            ...
            autoFocus={false}
            className={classNames(Classes.DATE_INPUT, popoverNextProps.className, props.className)}
            content={popoverContent}
            enforceFocus={false}
            onClose={handlePopoverClose}
            popoverClassName={classNames(Classes.DATE_INPUT_POPOVER, popoverNextProps.popoverClassName)}
            ref={popoverRef}
...
DateInputNext.displayName = `${DISPLAYNAME_PREFIX}.DateInputNext`;

/** Gets the input `placeholder` value from props, using default values if undefined */
function getPlaceholder(props: DateInputNextProps): string | undefined {
...
function getInitialTimezoneValue({ defaultTimezone, timezone }: DateInputNextProps) {
